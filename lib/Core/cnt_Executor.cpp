...
// here 
      if (func.compare("__get_sym_str") != 0) {
	    ...
	    //klee_message("no br. func:%s, id:%d", func.c_str(), ki->info->id);
	  }
        }
      }
...
klee_message(" *** start klee !! *** ");
  while (!states.empty() && !haltExecution) {
    ExecutionState &state = searcher->selectState();
    klee_message("-------------selected State!--------------\nthe state's all const:");
    for (auto i=state.constraints.begin();i!=state.constraints.end();++i) {
      klee_message(" ");
      llvm::errs() << *i << "\n";
    }
  ...
  klee_warning("cnt1:%d, 2:%d, 3:%d, 4:%d, 5:%d, 6:%d, 7:%d", cnt1, cnt2, cnt3, cnt4, cnt5, cnt6, cnt7);
