...
void BFSSearcher::update(ExecutionState *current,
  ...
  for (std::vector<ExecutionState *>::const_iterator it = removedStates.begin(),                    
    ...
    if (es == states.front()) {
      ...
      for (std::deque<ExecutionState*>::iterator it = states.begin(), ie = states.end(); it != ie; ++it) {
        if (es==*it) {
          states.erase(it);
          ok = true;
          break;
...
ParameterizedSearcher::ParameterizedSearcher(Executor &_executor, const std::string& weightFile)
 : doUpdate(false), executor_(_executor)
{
  double weight;
  ...
  assert(weights_.size() == 28 && "weight file size error");
  ...
  dyFeatures_.push_back(new IsFreshBranch(1));
  ...
  dyFeatures_.push_back(new IsInMostLargestNumOfUncovBr(executor_));
  dyFeatures_.push_back(new IsInMostRecentlyReachedFunc());
  dyFeatures_.push_back(new InstCount(0));
  dyFeatures_.push_back(new InstCount(1));
  dyFeatures_.push_back(new CPInstCount(0));
  dyFeatures_.push_back(new CPInstCount(1));
